...
- 所有字段都只基于输入证据，不要编。
- project_signal 只记录“用户自己的稳定工作容器”：用户明确在做/建设/推进的产品、研究计划、项目或 workstream。
- 不要把外部文章标题、外部产品名、网页标题、一次性调研对象、临时类比写成 project_signal。外部对象最多进入 active_question_signal 或 theme_signal。
- 如果用户只是说“这个外部产品像我们之前的想法/项目”，除非他明确说这个项目正在变化，否则 project_signal.has_signal=false。
- project_signal.project_name 必须是用户自己的项目名或工作流名称；没有明确名称就不要生成 project_signal。
...
{{
  ...
  "project_signal": {{
    ...
    "summary": "",
    "confidence": 0.0,
    "decision": "none|candidate_project_update|ignore_external_reference"
  }},
  "profile_signal": {{
    "has_signal": false,
    "summary": "",
